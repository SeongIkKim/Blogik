...
이에 관해서 설명해놓은 [아마존의 정책평가로직 관련 공식문서](https://docs.aws.amazon.com/ko_kr/IAM/latest/UserGuide/reference_policies_evaluation-logic.html)를 꼭 읽어보도록 하자.
</Info>
<Info>
AWS account가 나뉘어서 권한이 들어가는 경우 Cross Account라고 하는데,
이 경우 policy evaluation logic이 AND조건으로 바뀐다.
이에 대해서는 [AWS re:invent ninja 영상](https://www.youtube.com/watch?v=aISWoPf_XNE&ab_channel=AmazonWebServices)을 참고할것.
</Info>
...
* IAM user
  ...
  * user를 생성한 뒤 설정해야 하는것 
    * `password`
    * `mfa(멀티팩터 인증)`
    * 이게 없으면 콘솔에 로그인 할 수 없으므로 실제 패스워드 설정은 aws cli나 콘솔로 설정해주어야한다.
