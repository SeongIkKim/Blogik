...
numpy의 inner는 수식에서 말하는 내적과는 다르다. 수식에서는 보통 $tr(XY^T)$를 내적으로 계산한다. 그러나 inner는 두 행렬의 행 벡터만을 대상으로 한다.
...
[위의 코드에 나오는 intercept항, 즉 bias term을 추가하는 것은 computational graph를 만들고 gradient descent를 이용해 해를 찾을 때 사용하는 트릭이라는데, 더 공부해본 뒤 보충해야 할 듯하다.]
