...
또한, 이러한 **덧셈 연산이 역전파과정에서 Gradient를 복사하여 매번 전달해주는 역할**을 하므로,, RNN에 비해 멀리있는 timestep의 gradient까지 큰 변형없이 전달하게 되어 결과적으로 **long-term depandency를 확보**하였다.
